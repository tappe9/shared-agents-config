...
| doctor | override・明示した外部skillパス等を読み取り診断 | 0診断対象に問題なし、1実際の警告、2確認できた必須依存不足等 |
...
`doctor` は詳細な環境inventoryを正常終了の前提にしません。Codex CLI版や外部skillの導入元・版を取得できないこと、`local.toml` に外部skillパスを記入していないことだけでは警告にしません。明示した外部skillパスが欠落している場合や、override・設定不正など実際に確認できた問題だけを警告・エラーとして扱います。終了コード0は診断した範囲に問題がないことだけを表し、Codexセッションでのskill読み込み・role起動・model・sandboxの動作保証ではありません。外部skillの自動インストール、ログイン、モデル問い合わせは行いません。
