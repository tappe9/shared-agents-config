...
def _local(layout: Layout) -> dict:
    ...
    if set(data) - {'schema_version', 'superpowers', 'git_commit'}:
        raise ValueError('invalid-local-config')
    for section in ('superpowers', 'git_commit'):
...
def _cli() -> Diagnostic:
    ...
    if binary is None:
        return Diagnostic('codex-cli', 'unknown', 'not_checked', 'Codex CLI version was not checked.')
    try:
        ...
        return Diagnostic('codex-cli', 'unknown', 'not_checked', 'Codex CLI version could not be determined.')
...
def inspect_runtime(layout: Layout) -> list[Diagnostic]:
    ...
    for name in manifest['required_skills']:
        source = (local.get('superpowers', {}).get('skills_root')
                  if name.startswith('superpowers:')
                  else local.get('git_commit', {}).get('skill_file'))
        if source is None:
            continue
        path = (Path(source) / name.split(':', 1)[1] / 'SKILL.md'
                if name.startswith('superpowers:')
                else Path(source))
        ...
        try:
            ...
            checks.append(Diagnostic(
                name, 'ok' if exists else 'missing', 'observed',
                'Declared skill file exists.' if exists else 'Declared required skill file is missing.',
            ))
        except (OSError, ValueError):
            checks.append(Diagnostic(
                name, 'warn', 'not_checked',
                'Declared skill path cannot be safely inspected.',
            ))
    ...
    try:
        ...
        checks.append(Diagnostic(
            'global-override', 'warn' if active else 'ok', 'observed',
            'Nonempty override may hide shared instructions.'
            if active else 'No nonempty global override found.',
        ))
    except (OSError, ValueError):
        checks.append(Diagnostic(
            'global-override', 'warn', 'not_checked',
            'Override could not be inspected.',
        ))
    for relative in ('.codex/config.toml', '.codex/agents', '.agents/skills'):
        path = layout.repo / relative
        if path.exists() or is_link(path):
            checks.append(Diagnostic(
                'project-scope:' + relative, 'warn', 'observed',
                'Project scope may change effective configuration.',
            ))
    if is_link(layout.skills_home / 'superpowers'):
        checks.append(Diagnostic(
            'legacy-superpowers', 'warn', 'observed',
            'Legacy skill link exists; review duplicate discovery.',
        ))
    ...
    try:
        ...
        if config.is_file():
            ...
            uncertain = any(
                not isinstance(e.get('path'), str)
                or not Path(e['path']).is_absolute()
                or not Path(e['path']).is_file()
                for e in entries
            )
            if uncertain:
                checks.append(Diagnostic(
                    'skill-overrides', 'warn', 'observed',
                    'Relative or absent skill override targets need review.',
                ))
    except (OSError, ValueError, TypeError, AttributeError):
        checks.append(Diagnostic(
            'skill-overrides', 'warn', 'not_checked',
            'Skill override configuration cannot be inspected.',
        ))
    return checks
